...
#print_r($_POST); exit;
	if ($_POST['logar']==1) { 
		...
		$usuario = db_lista(db_consulta("SELECT id_usuario,nome,login,email,DATE_FORMAT(data_login,'%d/%m/%Y �s %H:%i:%s') as data_login,flag_status FROM adm_usuarios WHERE login LIKE '".$_POST['login']."' AND senha LIKE '".md5($_POST['senha'])."' LIMIT 1;"));
		if ($usuario['id_usuario']>0) {
			if ($usuario['flag_status']==1 || $usuario['id_usuario']==1) {
				...
				echo "<script language=javascript>alert('Usu�rio bloqueado.');window.location='_login.php';</script>"; 
				exit;
			}
		} else {
