...
<div id="box">
  ...
  <div id="content">
    ...
    <div id="conteudo">
        <span class="caminho">Home &raquo; Editar Usuário</span>

<?php if(isset($_POST['editar']) && $_POST['editar'] == 'ok'){
	
	 $id_do_usuario = $_POST['user_id'];

	 $senha   = strip_tags(trim($_POST['senha']));
	 $nome    = strip_tags(trim($_POST['nome']));
	 $email   = strip_tags(trim($_POST['email']));
	 
	 $atualizar_perfil = mysql_query("UPDATE up_users SET senha = '$senha', nome = '$nome', email = '$email' WHERE id = '$id_do_usuario'")
	                     or die(mysql_error());
...
?>
	 ...
	 $perfil = mysql_query("SELECT id, senha, nome, email FROM up_users WHERE id = '$id_do_usuario'")
	                  or die(mysql_error());
